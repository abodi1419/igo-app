...
@section('content')
    ...
    <div class="container">
        <div class="card border-success">
            <div class="card-header text-bg-success fw-bold">{{ __('Edit product') }}</div>
...
{{--                            </div>--}}
                            ...
                            <button class="btn btn-danger" onclick="ajaxDelete('{{route('image.delete',$image->id)}}')">
                ...
                <form action="{{route('products.update',$product)}}" method="post" enctype="multipart/form-data">
                    ...
                    <div class="row mb-3 text-center">
                        ...
                        @forelse($options as $option)
                            <div class="text-center">
                                ...
                                <label for="option{{$option->id}}" class="form-check-label">{{ app()->getLocale()=='ar' ? $option->option_ar: $option->option }}</label>
                            </div>
                        @empty
                            <div>
                                {{__('No options created ')}}<a class="d-inline" href="#">create one</a>
                            </div>
                        @endforelse
                    </div>
                    <div class="row mb-3 text-center">
                        ...
                        @forelse($productsCategories as $productsCategory)
                            <div class="text-center">
                                ...
                                <label for="category{{$productsCategory->id}}" class="form-check-label">{{  app()->getLocale()=='ar' ?$productsCategory->name_ar: $productsCategory->name }}</label>
                            </div>

                        @empty
                            <div>
                                {{__('No categories created ')}}<a class="d-inline" href="#">create one</a>
                            </div>
                        @endforelse
                    ...
                    <div class="row mb-3 text-center">
                        ...
                        @empty
                            <div>
                                {{__('No menus created ')}}<a class="d-inline" href="{{route('menus.create')}}">create one</a>
                            </div>
                        @endforelse
                        @error('menus')
                        <span class="invalid-feedback" role="alert">
