...
class ProductController extends Controller
{
    ...
    public function index(User $restaurant){
        $userProducts = $restaurant->products;
//        dd($user);
        return view('admin.products.index',compact('userProducts','restaurant'));
    }
    ...
    public function create(User $restaurant){
        ...
        return view('admin.products.create',compact('menus','options','productsCategories','restaurant'));
    }
...
//        dd($restaurant);
//        dd($request->all());
        $product = $restaurant->products()->create($validatedData);
        if($request->has('images')) {
            $images = [];
            foreach ($validatedData['images'] as $file) {
                $fileName = $file->hashName();
                $file->storeAs('public/restaurants/'. $restaurant->id.'/products/pr-' . $product->id, $fileName);
     ...
     * @return \Illuminate\Contracts\Foundation\Application|\Illuminate\Contracts\View\Factory|\Illuminate\Contracts\View\View
     */
    public function edit(Product $product,User $restaurant){
        $menus = $restaurant->menus;
        $productsCategories = $restaurant->productsCategories;
        $options = $restaurant->options;
        return view('admin.products.edit',compact('product','restaurant','menus','options','productsCategories'));
    }
    /**
     * @param Product $product
     ...
     * @return mixed
     */
    public function update(Product $product, User $restaurant, Request $request){
        $validatedData = $request->validate([
            'name'=>'string|required|max:255',
            'name_ar'=>'string|required|max:255',
            'description'=>'',
...
//        return $validatedData;
    ...
    public function show(Product $product, User $restaurant){
